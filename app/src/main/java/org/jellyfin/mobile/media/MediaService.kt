...
class MediaService : MediaBrowserServiceCompat() {
    ...
    private val libraryBrowser = LibraryBrowser(apiClient)
    ...
    override fun onGetRoot(
        ...
        val browserRootPath = if (isRecentRequest) LibraryContentType.Recent.name else LibraryContentType.Libraries.name
    ...
    override fun onLoadChildren(parentId: String, result: Result<List<MediaItem>>) {
        ...
        serviceScope.launch(Dispatchers.IO) {
            ...
            loadingJob.join()
            if (serverUser != null) {
                val idFields = parentId.split('|')
                try {
                    val (type, primaryItemId, secondaryItemId) = when (idFields.size) {
                        1 -> Triple(LibraryContentType.valueOf(idFields[0]), null, null)
                        2 -> Triple(LibraryContentType.valueOf(idFields[0]), idFields[1], null)
                        3 -> Triple(LibraryContentType.valueOf(idFields[0]), idFields[1], idFields[2])
                        else -> throw IllegalArgumentException()
                    }

                    val metadataList = libraryBrowser.loadLibrary(this@MediaService, type, primaryItemId, secondaryItemId)

                    val mediaItems = metadataList.map { metadata ->
                        MediaItem(metadata.description, type.flags)
                    }
    ...
    private inner class MediaPlaybackPreparer : MediaSessionConnector.PlaybackPreparer {
        ...
        override fun onPrepareFromMediaId(mediaId: String, playWhenReady: Boolean, extras: Bundle?) {
            /*val shouldShuffle = mediaId.endsWith(LibraryType.Shuffle.toString())
            serviceScope.launch {
                val metadataList = libraryBrowser.loadLibrary()
                preparePlaylist(metadataList, 0, playWhenReady)
            }*/
        }
